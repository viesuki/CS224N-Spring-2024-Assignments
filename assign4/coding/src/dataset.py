...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        len_doc = len(truncated_doc)
        len_mask = len_doc // 4 + random.randint(-len_doc // 8, len_doc // 8)
        idx_start = random.randint(0, len_doc - len_mask)
        prefix, masked_content, suffix = (
            truncated_doc[:idx_start],
            truncated_doc[idx_start:idx_start + len_mask],
            truncated_doc[idx_start + len_mask:]
        )
