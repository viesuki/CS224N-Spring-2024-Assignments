...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    position = torch.arange(0, max_positions).unsqueeze(1)  # (max_positions, 1)
    dim_indices = torch.arange(0, dim // 2).unsqueeze(0)  # (1, dim/2)        
    theta = 1 / 10000 ** (dim_indices * 2 / dim)
    rope_cache[:, :, 0] = torch.cos(position * theta)  # (max_positions, dim/2)
    rope_cache[:, :, 1] = torch.sin(position * theta)  # (max_positions, dim/2)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    ### YOUR CODE HERE ###
    B, nh, T, hs = x.size()
    x_reshaped = x.reshape(-1, T, hs // 2, 2)  # (B*nh, T, hs/2, 2)
    x_complex = torch.view_as_complex(x_reshaped)  # (B*nh, T, hs/2)
    rope_complex = torch.view_as_complex(rope_cache).unsqueeze(0)  # (1, T, hs/2)
    # truncate rope_complex if T is less than max_positions
    rope_complex = rope_complex[:, :T, :]  # (1, T, hs/2)
    rotated_x = x_complex * rope_complex  # (B*nh, T, hs/2)
    rotated_x = torch.view_as_real(rotated_x).view(B, nh, T, hs)  # (B, nh, T, hs)
    ### END YOUR CODE ###
    return rotated_x
