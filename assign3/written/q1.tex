...
\end{lstlisting}
    ...
    \ifans{\\
    The masks are used to prevent the model from attending to padding tokens, which do not contain useful information. By masking these tokens, we ensure that the attention mechanism focuses only on the actual content of the input sequences.
    }
...
\end{lstlisting}
    ...
    \ifans{
    \begin{enumerate}
        \item \textbf{Advantage}: Dot product attention is computationally efficient as it involves only a simple dot product operation, making it faster to compute. \textbf{Disadvantage}: It may not capture complex relationships between the query and key vectors as effectively as multiplicative attention, which includes a learned weight matrix.
        \item \textbf{Advantage}: Additive attention can model more complex interactions between the query and key vectors due to the use of learned weight matrices and a non-linear activation function even when the query, key, and value have different dimensions. \textbf{Disadvantage}: It is computationally more expensive than multiplicative attention, as it involves additional matrix multiplications and a non-linear activation, which can slow down training and inference.
    \end{enumerate}
    }
