...
if "%1%"=="train" (
    set CUDA_VISIBLE_DEVICES=0 & python run.py train --train-src=./zh_en_data/train.zh --train-tgt=./zh_en_data/train.en --dev-src=./zh_en_data/dev.zh --dev-tgt=./zh_en_data/dev.en --vocab=vocab.json --cuda --lr=5e-4 --patience=10 --valid-niter=200 --batch-size=32 --dropout=.3 --max-epoch=10
) else if "%1%"=="test" (
