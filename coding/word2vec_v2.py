...
class TextPreprocessor:
    ...
    def sentences_to_indices(self, text):
        ...
        indices_per_sent = []
        for s in sentences:
            words = self.preprocess_text(s)        # 这里会去标点、转小写
...
class NegativeSamplingTrainer:
    ...
    def get_negative_samples(self, batch_size, exclude_indices=None, device='cpu'):
        """生成负样本"""
        negative_samples = torch.multinomial(
            self.neg_distribution.to(device), 
        ...
        if exclude_indices is not None:
            # exclude_indices 形状应为 [batch]，逐行重采直到不等于正样本
            exclude_indices = exclude_indices.view(-1, 1).expand_as(negative_samples)
            mask = negative_samples.eq(exclude_indices)
            # 迭代重采，直到无冲突
            while mask.any():
                ...
                negative_samples[mask] = resample.squeeze(1)
                mask = negative_samples.eq(exclude_indices)

        return negative_samples.to(device)
...
class Word2VecTrainer:
    ...
    def prepare_training_data(self):
        ...
        for text in self.corpus:
            sent_indices_list = self.preprocessor.sentences_to_indices(text)
            for indices in sent_indices_list:
        ...
        if self.model_type == 'skipgram':
            return torch.tensor(all_training_data, dtype=torch.long)  # [num_samples, 2]
        else:
            return all_training_data  # list of (List[int], int)
    ...
    def train(self, epochs=10, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            if self.model_type == 'skipgram':
                ...
                perm = torch.randperm(len(training_data), device=self.device)
                ...
                for i in range(0, len(training_data), batch_size):
                    ...
                    negative_samples = self.trainer.get_negative_samples(
                        len(batch_data), context_words, device=self.device)
                    # 前向传播
