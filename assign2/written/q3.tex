...
\begin{parts}
    ...
    \part[4] Go through the sequence of transitions needed for parsing the sentence {\it ``I presented my findings at the NLP conference''}. The dependency tree for the sentence is shown below. At each step, give the configuration of the stack and buffer, as well as what transition was applied this step and what new dependency was added (if any). The first three steps are provided below as an example.
    ...
    \ifans{ 
        \begin{adjustwidth}{-2cm}{} 
            \begin{tabular}{ p{4.2cm} | p{5.2cm} | l | l}
                Stack & Buffer & New dependency & Transition \\ \hline 
                [ROOT] & [I, presented, my, findings, at, the, NLP, conference] & & Initial Configuration \\ 
                $[$ROOT, I] & [presented, my, findings, at, the, NLP, conference] & & \texttt{SHIFT} \\ 
                $[$ROOT, I, presented] & [my, findings, at, the, NLP, conference] & & \texttt{SHIFT} \\ 
    ...
    % Answer 2
    \ifans{ \\
        A sentence containing $n$ words will be parsed in $2n$ steps. This is because each word must be shifted onto the stack (which takes $n$ steps), and then each word must be removed from the stack via either a LEFT-ARC or RIGHT-ARC operation (which takes another $n$ steps).
    }
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \ifans{
    \begin{enumerate}[label=\roman*.]
        \item $\frac{\partial h_i}{\partial x_j} = \begin{cases} W_{j,i}, & \text{if } \bx \bW + \bb_1 > 0 \\ 0, & \text{otherwise} \end{cases}$ 
        \item $\frac{\partial CE(\by, \byt)}{\partial \bl_i} = \begin{cases} \byt_i - 1, & \text{if } i = c \\ \byt_i, & \text{otherwise} \end{cases}$ (i.e., $\byt - \by$)
        \item Scores        
    \end{enumerate}
        \begin{tabular}{ l | l | l }
            \textbf{Metric} & \textbf{Score default} & \textbf{Score 20/256} \\
            \hline
            Train loss & 0.0688 & wait \\
            Dev UAS & 88.52 & wait \\
            Test UAS & 89.42 & wait \\
        \end{tabular}
    }
...
\ifans{\\
    \begin{tabular}{llll}
        \textbf{No.} & \textbf{Error type} & \textbf{Incorrect dependency} & \textbf{Correct dependency} \\
        \hline
        1 & Verb Phrase Attachment Error & acquisition $\rightarrow$ citing & blocked $\rightarrow$ citing \\
        2 & Modifier Attachment Error & elements $\rightarrow$ most & crucial $\rightarrow$ most \\
        3 & Prepositional Phrase Attachment Error & declined $\rightarrow$ decision & reasons $\rightarrow$ decision \\
        4 & Coordination Attachment Error & affects $\rightarrow$ one & plants $\rightarrow$ one \\
    \end{tabular}
}
...
\ifans{\\
    Part-of-speech tags are useful because they provide a way to distinguish between different types of words. For example, the word \textit{dog} can be a noun, verb, adjective, etc. In the parser, we can use the part-of-speech tag of a word to distinguish between different types of words. So we can use the fact that \textit{dog} is a \textit{noun} to distinguish it from other words that are not nouns. This can help the parser make better decisions about which words to attach to which heads.
}
